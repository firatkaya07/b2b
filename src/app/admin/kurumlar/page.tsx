...
export default async function InstitutionsPage() {
  ...
  const institutions = await q<{ id: number; name: string; city: string; contact_name: string }>(
    "SELECT * FROM institutions ORDER BY name"
  );
  const reps = await q<{ full_name: string; phone: string; institution_id: number }>(
    "SELECT full_name, phone, institution_id FROM users WHERE role='institution'"
  );
